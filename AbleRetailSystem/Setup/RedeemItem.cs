...
namespace RestaurantPOS.Inventory
{
    ...
    public partial class RedeemItem : Form
    {
        ...
        private void SearchItems(string pWhereCol, string pSearchText, ref DataGridView pDgrid, bool isLoad = true, bool isForRedeem = false)
        {
            ...
            if (isForRedeem)
            {
                selectSql = " SELECT pi.ItemID, pi.id as PromoID, ISNULL(pi.requiredpoints,0) as requiredpoints, pi.promotype, gc.GCAmount, gc.GCNumber, gc.StartDate, gc.EndDate, gc.IsUsed , gc.ID as GCID ";
                selectSql += @"FROM PromotionItems pi LEFT JOIN GiftCertificate gc ON pi.ID = gc.promoid";
