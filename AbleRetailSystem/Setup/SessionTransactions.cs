...
namespace RestaurantPOS.Setup
{
    public partial class SessionTransactions : Form
    {
        ...
        void SessionCalc()
        {
            ...
            {
                if (this.dgSession.Rows[i].Cells["TotalPaid"].Value != null)
                {
                    if (this.dgSession.Rows[i].Cells["TotalPaid"].Value.ToString() != "")
                    {
                        amt += decimal.Parse(dgSession.Rows[i].Cells["TotalPaid"].Value.ToString());
                    }
                }
            }
        ...
        private void LoadReport()
        {
            ...
            string printSessionSummaryReport = @"SELECT s.*, cps.TotalCount, (cps.TotalCount - s.Amount ) as Discrepancy, SessionKey, SessionStart, SessionEnd from 
                        (SELECT  SUM(pt.Amount) as Amount, pd.PaymentMethodID, pm.PaymentMethod, p.SessionID FROM PaymentTender pt 
                        ...
                        WHERE pt.Amount <> 0 and p.SessionID in ( @SessionID) group by pd.PaymentMethodID, pm.PaymentMethod,p.SessionID ) s 
						left join (SELECT distinct SessionID, PaymentMethodID, TotalCount from CountPerSession where SessionID in ( @SessionID)) cps ON cps.SessionID = s.SessionID and s.PaymentMethodID = cps.PaymentMethodID
						left join Sessions ss on s.SessionID = ss.SessionID";
