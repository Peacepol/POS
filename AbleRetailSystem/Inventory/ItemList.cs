    ...
    public partial class ItemList : Form
    {
        ...
        private void SearchItems(string pWhereCol, string pSearchText)
        {
            ...
            IsLoading = true;
                ...
                {
                case "Part Number":
                    strCon = " where ( i.PartNumber LIKE '%" + pSearchText + "%' or bc.BarcodeData LIKE '%" + pSearchText + "%' )"; 
                    break;
                case "Item Number":
                    strCon = " where i.ItemNumber LIKE '%" + pSearchText + "%'";
                    break;
                case "Supplier Part Number":
                    strCon = " where i.SupplierPartNumber LIKE '%" + pSearchText + "%'";
                    break;
                case "Item Name":
                    strCon = " where i.ItemName LIKE '%" + pSearchText + "%'";
                    break;
                case "Item Description":
                    strCon = " where i.ItemDescriptionSimple LIKE '%" + pSearchText + "%'";
                    break;
                case "Supplier":
                    strCon = " where p.Name LIKE '%" + pSearchText + "%'";
                    break;
                case "Category":
                    strCon = " where g.CategoryID in (" + categories + ")";
                    break;
                case "Barcode":
                    strCon = " where bc.BarcodeData LIKE '%" + pSearchText + "%' ";
                    break;
                case "Custom List 1":
                    strCon = " where c1.List1Name LIKE '%" + pSearchText + "%' ";
                    break;
                case "Custom List 2":
                    strCon = " where c2.List2Name LIKE '%" + pSearchText + "%' ";
                    break;
                case "Custom List 3":
                    strCon = " where c3.List3Name LIKE '%" + pSearchText + "%' ";
                    break;
                default:
            ...
            if (txtPartNum.Text != "")
            {
                strCon += " OR i.PartNumber LIKE '%" + txtPartNum.Text + "%' ";
            }
            if (txtItemNum.Text != "")
            {
                strCon += " OR i.ItemNumber LIKE '%" + txtItemNum.Text + "%' ";
            }
            if (categories != "")
            {
            ...
            //if (txtMainCat.Text != "")
            //{
            //    strCon += "OR g.CategoryCode LIKE '%" + txtMainCat.Text + "%' AND g.CategoryID = '" + txtMainCatID.Text + "'";
            //}
            if (txtDesc.Text != "")
            {
                strCon += " OR i.ItemDescriptionSimple LIKE '%" + txtDesc.Text + "%' ";
            }
            if (txtSupplier.Text != "")
            {
                strCon += " OR p.Name LIKE '%" + txtSupplier.Text + "%' ";
            }
            if (txtBrand.Text != "")
            {
                strCon += " OR i.PartNumber LIKE '%" + txtBrand.Text + "%' ";
            }
            if (txtSuppPartNum.Text != "")
            {
                strCon += " OR i.SupplierPartNumber LIKE '%" + txtSuppPartNum.Text + "%' ";
            }
            if (txtItemName.Text != "")
            {
                strCon += " OR i.ItemName LIKE '%" + txtItemName.Text + "%' ";
            }
            if (txtSerialNum.Text != "")
            {
                strCon += " OR bc.BarcodeData LIKE '%" + txtSerialNum.Text + "%'  ";
            }
            if (txtCustomList1.Text != "")
            {
                strCon += " OR c1.List1Name LIKE '%" + txtCustomList1.Text + "%' ";
            }
            if (txtCustomList2.Text != "")
            {
                strCon += " OR c2.List2Name LIKE '%" + txtCustomList2.Text + "%' ";
            }
            if (txtCustomList3.Text != "")
            {
                strCon += " OR c3.List3Name LIKE '%" + txtCustomList3.Text + "%' ";
            }

            string selectSql = @"Select i.ID, i.PartNumber, i.ItemNumber, i.ItemName, q.OnHandQty, c.LastCostEx, s.Level0 from 
                    ...
                    LEFT JOIN CustomList3 c3 ON c3.ID = i.CList3 " + strCon;
