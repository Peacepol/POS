    ...
    public partial class SessionReportsCustomizer : Form
    {
        ...
        private void LoadSummaryReport()
        {
            ...
            string printSessionSummaryReport = @"SELECT s.*, cps.TotalCount, (cps.TotalCount - s.Amount ) as Discrepancy, SessionKey, SessionStart, SessionEnd from 
                        ...
                        WHERE pt.Amount <> 0 and p.SessionID in ( " + ID + ") group by pd.PaymentMethodID, pm.PaymentMethod,p.SessionID ) s"; 						

             printSessionSummaryReport += @" left join (SELECT distinct SessionID, PaymentMethodID, TotalCount from CountPerSession where SessionID in ( " + ID + ")) cps ON cps.SessionID = s.SessionID and s.PaymentMethodID = cps.PaymentMethodID";
            printSessionSummaryReport += @" left join Sessions ss on s.SessionID = ss.SessionID where ss.SessionID in ( " + ID + ")";
