...
namespace RestaurantPOS.Sales
{
    public partial class QuickSales : Form
    {
        ...
        public void ShowItemLookup(string itemNum, string whereCon)
        {
            ...
            DataGridViewRow dgvRows = dgEnterSales.Rows[rowin];
                ...
                if (this.dgEnterSales.Rows[i].Cells["Description"].Value != null)
                {
                    if (this.dgEnterSales.Rows[i].Cells["Description"].Value.ToString() != "")
                    {
                            ...
                            if (lItemTb.Rows.Count > 0)
                            {
                                ...
                                if (lIsCounted)
                                {
                                    ...
                                    salesLinesql = @"INSERT INTO ItemTransaction(TransactionDate,ItemId,TransactionQty,QtyAdjustment,CostEx,TotalCostEx,TranType,SourceTranID,UserID) 
                                                    VALUES('" + lTranDate + "'," + entity + "," + ShipQty + "," + ShipQty + " *(-1)," + CostPrice + "," + TotalCost + ",'SI'," + NewSalesID + "," + CommonClass.UserID + ")";
                        ...
                        salesLinesql = "INSERT INTO SalesLines (SalesID, Description, TotalAmount, TransactionDate, TaxCode, UnitPrice, ActualUnitPrice, DiscountPercent, OrderQty, ShipQty, EntityID, JobID, SubTotal, TaxAmount, TaxCollectedAccountID,TaxRate, CostPrice, TotalCost, PromoID)" +
                                       " VALUES (" + NewSalesID + ", '" + Descript + "', '" + dAmount + "','" + lTranDate + "', '" + Tax + "', '" + UnitPrice + "', '" + ActualPrice + "', '" + Discount + "', '" + OrderQty + "', '" + ShipQty + "', '" + entity + "', '" + Job + "', " + taxEx + ", " + TaxAm + ", '" + TaxID + "'," + TaxRate + "," + CostPrice + "," + TotalCost + ", " + promoid + ")";

                        count = CommonClass.runSql(salesLinesql, CommonClass.RunSqlInsertMode.SCALAR);
        ...
        private int CreatePaymentRecord(string pPaymentNo, string pSource = "")
        {
            ...
            try
            {
                ...
                {//FullPayment
                    ...
                    //    paramPayLines.Add("@ChangeAmount", change);
                    //    CommonClass.runSql(strpaymentlnesql, CommonClass.RunSqlInsertMode.QUERY, paramPayLines);
                    //}
        ...
        void LoadPaymentLogo(string PayM, int index)
        {
            ...
            {
                case "Cash"://cash
                    img = new Bitmap(global::RestaurantPOS.Properties.Resources.Cash);
                    ResizeImg = new Bitmap(img, width, height);
                    dgPaymentMethod.Rows[index].Cells[1].Value = ResizeImg;
                    break;

                case "Cheque": //cheque
                    img = new Bitmap(global::RestaurantPOS.Properties.Resources.Cheque);
                    ResizeImg = new Bitmap(img, width, height);
                    dgPaymentMethod.Rows[index].Cells[1].Value = ResizeImg;
                    break;
                case "Complimentary": //cheque
                    img = new Bitmap(global::RestaurantPOS.Properties.Resources.business_and_finance);
                    ResizeImg = new Bitmap(img, width, height);
                    dgPaymentMethod.Rows[index].Cells[1].Value = ResizeImg;
                    break;

                case "Credit Card": //credit
                    img = new Bitmap(global::RestaurantPOS.Properties.Resources.credit_card);
                    ResizeImg = new Bitmap(img, width, height);
                    dgPaymentMethod.Rows[index].Cells[1].Value = ResizeImg;
                    break;
                case "Debit Card": //Debit Card
                    img = new Bitmap(global::RestaurantPOS.Properties.Resources.pay);
                    ResizeImg = new Bitmap(img, width, height);
                    dgPaymentMethod.Rows[index].Cells[1].Value = ResizeImg;
                    break;
                case "Direct Deposit":  //
                    img = new Bitmap(global::RestaurantPOS.Properties.Resources.DirectDeposit);
                    ResizeImg = new Bitmap(img, width, height);
                    dgPaymentMethod.Rows[index].Cells[1].Value = ResizeImg;
                    break;
                case "Eftpos":   //
                    img = new Bitmap(global::RestaurantPOS.Properties.Resources.eftpos);
                    ResizeImg = new Bitmap(img, width, height);
                    dgPaymentMethod.Rows[index].Cells[1].Value = ResizeImg;
                    break;
                case "GC":   //GC
                    img = new Bitmap(global::RestaurantPOS.Properties.Resources.gift_card);
                    ResizeImg = new Bitmap(img, width, height);
                    dgPaymentMethod.Rows[index].Cells[1].Value = ResizeImg;
                    break;
                case "Other":    //
                    img = new Bitmap(global::RestaurantPOS.Properties.Resources.Others);
                    ResizeImg = new Bitmap(img, width, height);
                    dgPaymentMethod.Rows[index].Cells[1].Value = ResizeImg;
                    break;

                case "Paypal":   //paypal
                    img = new Bitmap(global::RestaurantPOS.Properties.Resources.paypal);
                    ResizeImg = new Bitmap(img, width, height);
                    dgPaymentMethod.Rows[index].Cells[1].Value = ResizeImg;
                    break;

                case "Salary Sacrifice":    //
                    img = new Bitmap(global::RestaurantPOS.Properties.Resources.SalarySacrifice);
                    ResizeImg = new Bitmap(img, width, height);
                    dgPaymentMethod.Rows[index].Cells[1].Value = ResizeImg;
                    break;

                case "Staff Deduction":   //
                    img = new Bitmap(global::RestaurantPOS.Properties.Resources.staffDeduction);
                    ResizeImg = new Bitmap(img, width, height);
                    dgPaymentMethod.Rows[index].Cells[1].Value = ResizeImg;
                    break;
                case "Voucher":   //
                    img = new Bitmap(global::RestaurantPOS.Properties.Resources.coupon);
                    ResizeImg = new Bitmap(img, width, height);
                    dgPaymentMethod.Rows[index].Cells[1].Value = ResizeImg;
                    break;
        ...
        public void SetPartName(string pPartNum)
        {
            ...
            txtPartNum.Select();
            CalcOutOfBalance();
            if(lastFilledRow + 1 < dgEnterSales.Rows.Count)
            {
                dgEnterSales.FirstDisplayedScrollingRowIndex = lastFilledRow + 1;
            }
        ...
        {
            if (rowin < dgEnterSales.Rows.Count - 1)
            {
                dgEnterSales.Rows[rowin].Selected = false;
                dgEnterSales.Rows[++rowin].Selected = true;
            }
        ...
        private float ComputePromo(string accumulation, float points, float pointsvalue, float PurchasePrice, int promoID)
        {
            ...
            if (dgEnterSales.CurrentRow.Cells["ActualUnitPrice"].Value != null)
            {
        ...
        private void btnRedeem_Click(object sender, EventArgs e)
        {
            ...
            {
                CommonClass.PointRedemption = new PointsRedemption(CommonClass.RedemptionType.GIFTCERTIFICATE, null, this, CustomerID, customerText.Text);
                dgEnterSales.Rows.Add();
            }
            ...
            if (e.KeyChar == 13)
            {
                ...
                if (dt.Rows.Count >= 1)
                {
                    ...
                    if (today >= gcsdate || today <= gcedate)
                    {
                        ...
                        if (gcamount > 0)
                        {
                            dgPaymentMethod.Rows[7].Cells["TenderAmount"].Value = gcamount;
                            TenderCalc();
                            txtGCNotes.Focus();
                        }
                        else
        ...
        {
            if (dgEnterSales.Rows.Count > 0 && dgEnterSales.Rows[dgEnterSales.CurrentCell.RowIndex].Cells["PartNumber"].Value != null)
            {
                if (CommonClass.isSalesperson == true || CommonClass.isTechnician == true)
                {
                    ...
                    string pvaluestr = "";
                    string negativeVal = "";
                    KeyboardOnScreen kbos = new KeyboardOnScreen("Enter Discount Value", "Enter Discount Value : ");
                    if (kbos.ShowDialog() == DialogResult.OK)
                    {
                        pvaluestr = string.Concat(kbos.GetValue.ToString().Where(char.IsLetter));
                        negativeVal = string.Concat(kbos.GetValue.ToString().Where(char.IsLetter));
                        pvalue = String.Format("{0:0.##}", kbos.GetValue);
                    }
                    if (txtDiscount.Text != "" && pvaluestr == "")
                    {
                        if (negativeVal == "-")
                        {
                            txtDiscount.Value = decimal.Parse(negativeVal + kbos.GetValue);
                            dgEnterSales.Rows[rowin].Cells["Discount"].Value = negativeVal + pvalue;
                        }
                        else
                        {
                            txtDiscount.Value = decimal.Parse(kbos.GetValue);
                            dgEnterSales.Rows[rowin].Cells["Discount"].Value = pvalue;
                        }
                    }
                    else
                    {
                        MessageBox.Show("Invalid Discount Percent!");
                    }
                }
                else
                {
            ...
            if (dgEnterSales.Rows.Count > 0 && dgEnterSales.Rows[dgEnterSales.CurrentCell.RowIndex].Cells["PartNumber"].Value != null)
            {
                if (CommonClass.isSalesperson == true || CommonClass.isTechnician == true)
                {
                    ...
                    if (DlgVoid.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        if (txtDiscount.Text != "" && pvaluestr == "")
                        {
                            ...
                            rw["UserName"] = DlgVoid.GetUsername;
                            rw["AuditAction"] = "Discount override to " + txtDiscount.Value + " by " + DlgVoid.GetUsername;
                            dtv.Rows.Add(rw);
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    if (txtDiscount.Text != "" && pvaluestr == "")
                    {
                        ...
                        rw["AuditAction"] = "Discount override to " + txtDiscount.Value + " by " + CommonClass.UserName;
                        dtv.Rows.Add(rw);
                    }
                    else
                    {
        ...
        private void txtDiscount_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (vv.ShowDialog() == DialogResult.OK)
            {
                ...
                rw["UserName"] = vv.GetUsername;
                rw["AuditAction"] = "Discount override to " + txtDiscount.Value + " by " + vv.GetUsername;
                dtv.Rows.Add(rw);
            }
        }
    }//End
