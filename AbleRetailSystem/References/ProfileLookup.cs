        ...
        private void LoadProfile(string pSearch = "", bool activeOnly = false)
        {
            ...
            try
            {
                ...
                if (pSearch != "")
                {
                    ...
                    {
                        sqlcon = " and p.Name LIKE '%" + this.txtsearch.Text + "%'";
                    }
                    if (this.rdoID.Checked)
                    {
                        sqlcon = " and p.ProfileIDNumber LIKE '%" + this.txtsearch.Text + "%'";
                    }
                    if (this.rdoPhone.Checked)
                    {
                        sqlcon = " and c.Phone LIKE '%" + this.txtsearch.Text + "%'";
                    }
                    if (this.rdoEmail.Checked)
                    {
                        sqlcon = " and c.Email LIKE '%" + this.txtsearch.Text + "%'";
                    }
                    if (this.rdoCity.Checked)
                    {
                        sqlcon = " and c.City LIKE '%" + this.txtsearch.Text + "%'";
                    }
                    if (this.rdoState.Checked)
                    {
                        sqlcon = " and c.State LIKE '%" + this.txtsearch.Text + "%'";
                    }
                    if (this.rdoPostCode.Checked)
                    {
                        sqlcon = " and c.Postcode LIKE '%" + this.txtsearch.Text + "%'";
                    }
                    if (this.rdoCountry.Checked)
                    {
                        sqlcon = " and c.Country LIKE '%" + this.txtsearch.Text + "%'";
                    }
                    if (this.rdoLoyal.Checked)
                    {
                        sqlcon = " AND l.Number LIKE '%" + this.txtsearch.Text + "%'";
                    }
                    if (pSearch == "Active")
                    {
