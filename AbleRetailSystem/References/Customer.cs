        ...
        private void SearchTrans(string pDateFrom, string pDateTo, string pProfileID)
        {
            ...
            if (cbSalesType.Text == "QUOTE")
            {
                ...
                sql = @"SELECT SalesNumber AS TransactionNumber,
                    ...
                    INNER JOIN Journal j ON j.TransactionNumber = s.SalesNumber
                    WHERE CustomerID = '" + pProfileID + "'" +
                    " AND s.TransactionDate BETWEEN @sdate AND @edate AND SalesID = j.EntityID AND j.Type = 'SQ'";
            }
            else if (cbSalesType.Text == "ORDER")
            {
                sql = @"SELECT SalesNumber AS TransactionNumber,
                    ...
                    INNER JOIN Journal j ON j.TransactionNumber = s.SalesNumber
                    WHERE CustomerID = '" + pProfileID + "'" +
                    " AND s.TransactionDate BETWEEN @sdate AND @edate AND SalesID = j.EntityID AND j.Type = 'SO' ";
            }
            else if (cbSalesType.Text == "INVOICE")
            {
                sql = @"SELECT SalesNumber AS TransactionNumber,
                    ...
                    INNER JOIN Journal j ON j.TransactionNumber = s.SalesNumber
                    WHERE CustomerID = '" + pProfileID + "'" +
                    " AND s.TransactionDate BETWEEN @sdate AND @edate AND SalesID = j.EntityID AND j.Type = 'SI'";
            }
            else
            {
                //Payment
                ...
                //Sales
                sql += @"UNION SELECT SalesNumber AS TransactionNumber, 
                            s.TransactionDate, 
                            s.Memo,
                            GrandTotal AS Amount, 
                            j.Type,
                        ...
                        INNER JOIN Journal j ON j.TransactionNumber = s.SalesNumber 
                        WHERE CustomerID = '" + pProfileID + "'" +
                        " AND s.TransactionDate BETWEEN @sdate AND @edate AND SalesID = j.EntityID ";
            ...
            //ProfileIDNumber
            //               FROM Sales s INNER JOIN Profile p ON s.CustomerID = p.ID
            //               WHERE SalesType = 'SINVOICE' and CustomerID = '" + pProfileID + "'" +
            //               " AND s.TransactionDate BETWEEN @sdate AND @edate";
