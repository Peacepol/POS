...
namespace AbleRetailPOS
{
    public partial class CoProForm : Form
    {
        ...
        public CoProForm(MainPage mpref)
        {
            InitializeComponent();
            mainpageref = mpref;
        }
        ...
        {
            CreateNewCompany ncomp = new CreateNewCompany();
            if (ncomp.ShowDialog() == DialogResult.OK)
            {
                CoProForm_Load(sender, e);
            }
        }
        ...
        private void Login()
        {
            ...
            try
            {
                ...
                if (resultreader.HasRows)
                {
                    ...
                    {
                        string dbconame = resultreader["CompanyName"].ToString();
                        string lsconame = SelCoRow.Cells["CompName"].Value.ToString();
                        string dbrego = resultreader["CompanyRegistrationNumber"].ToString();
                        string lsrego = SelCoRow.Cells["RegistrationNo"].Value.ToString();
                        string dbsn = resultreader["SerialNumber"].ToString();
                        string lssn = SelCoRow.Cells["SerialNo"].Value.ToString();

                        if (dbconame == lsconame
                            && dbrego == lsrego
                            && dbsn == lssn)
                        {
        ...
        private void CoProForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (!mainpageref.Visible)
                mainpageref.Close();
        }
    }
}
