...
namespace AbleRetailPOS
{
    public partial class CreateNewCompany : Form
    {
        ...
        private void btnCompanyDatabase_Click(object sender, EventArgs e)
        {
            ...
            {
                int indexOfFirst;
                string tempComp = txtCompanyName.Text;
                if (txtCompanyName.Text.Substring(0).Contains("\'"))
                {
                    indexOfFirst = txtCompanyName.Text.IndexOf('\'');
                    tempComp = txtCompanyName.Text.Remove(indexOfFirst, 1);
                }
                int i = (tempComp + " ").IndexOf(" ");
