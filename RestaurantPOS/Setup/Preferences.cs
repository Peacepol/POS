...
namespace AbleRetailPOS
{
    public partial class Preferences : Form
    {
        ...
        DataTable dtPref;
        DataTable dtSeries;
        private int POSDelivery;
        private  int POSTakeAway;
        private int POSDineIn;
        public Preferences()
        {
            InitializeComponent();
...
RestoDineInShipping = @DineIn,
RestoDeliveryShipping = @Delivery,
RestoTakeAwayShipping = @TakeAway";
                ...
                updateparam.Add("@DineIn", POSDineIn);
                updateparam.Add("@Delivery", POSDelivery);
                updateparam.Add("@TakeAway", POSTakeAway);
            ...
            else
            {
                ...
                insertparam.Add("@DineIn", POSDineIn);
                insertparam.Add("@Delivery", POSDelivery);
                insertparam.Add("@TakeAway", POSTakeAway);
        ...
        private void Preferences_Load(object sender, EventArgs e)
        {
            ...
            string selprefsql = @"SELECT p.*, 
                                         c.CurrencySymbol, 
                                         Profile.Name, sa.ShippingMethod as DineIN ,sb.ShippingMethod as Delivery,sc.ShippingMethod as TakeAway 
                                    FROM Preference p 
                                    LEFT JOIN Currency c ON p.LocalCurrency = c.CurrencyCode 
                                    LEFT JOIN Profile ON Profile.ID = p.DefaultCustomerID 
                                    LEFT JOIn ShippingMethods sa ON sa.ShippingID = RestoDineInShipping
 LEFT JOIn ShippingMethods sb ON sb.ShippingID = RestoDeliveryShipping 
LEFT JOIn ShippingMethods sc ON sc.ShippingID = RestoTakeAwayShipping";
            ...
            if (dtPref.Rows.Count > 0)
            {
                ...
                txtDineIn.Text = x["DineIN"].ToString();
                txtDelivery.Text = x["Delivery"].ToString();
                txtTakeAway.Text = x["TakeAway"].ToString();
        ...
        private void pbDelivery_Click(object sender, EventArgs e)
        {
            ...
            if (DlgShippingMethod.ShowDialog() == DialogResult.OK)
            {
                ...
                txtDelivery.Text = ShipList[0];
                POSDelivery = int.Parse(ShipList[1]);
            }
        }

        private void pbTakeAway_Click(object sender, EventArgs e)
        {
            ShippingMethodLookup DlgShippingMethod = new ShippingMethodLookup();
            if (DlgShippingMethod.ShowDialog() == DialogResult.OK)
            {
                string[] ShipList = DlgShippingMethod.GetShippingMethod;
                txtTakeAway.Text = ShipList[0];
                POSTakeAway = int.Parse(ShipList[1]);
            }
        }

        private void pbDineIn_Click(object sender, EventArgs e)
        {
            ShippingMethodLookup DlgShippingMethod = new ShippingMethodLookup();
            if (DlgShippingMethod.ShowDialog() == DialogResult.OK)
            {
                string[] ShipList = DlgShippingMethod.GetShippingMethod;
                txtDineIn.Text = ShipList[0];
                POSDineIn = int.Parse(ShipList[1]);
            }
        }
    } //end
}
