...
using AbleRetailPOS.Reports.PurchaseReports;
using AbleRetailPOS.Reports.SalesReports;
using System;
...
namespace AbleRetailPOS
{
    public partial class ReportsNavigation : Form
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            if (listInventoryReports.SelectedIndex == 11)
            {
                if (CommonClass.salesIngredient == null || CommonClass.salesIngredient.IsDisposed)
                {
                    CommonClass.salesIngredient = new Reports.InventoryReports.SalesIngredient();
                }
                this.Cursor = Cursors.WaitCursor;
                CommonClass.salesIngredient.MdiParent = this.MdiParent;
                CommonClass.salesIngredient.Show();
                CommonClass.salesIngredient.Focus();
                if (CommonClass.salesIngredient.DialogResult == DialogResult.Cancel)
                {
                    CommonClass.salesIngredient.Close();
                }
                this.Cursor = Cursors.Default;
            }
           ...
           || CommonClass.PriceReportDetails.IsDisposed)
            ...
            if (listInventoryReports.SelectedIndex == 15)
            {
            ...
            if (listInventoryReports.SelectedIndex == 18)
            {
                if (CommonClass.ItemSalesAnalysis == null
                           || CommonClass.ItemSalesAnalysis.IsDisposed)
                {
                    CommonClass.ItemSalesAnalysis = new Reports.InventoryReports.RptSalesAnalysisItem();
                }
                this.Cursor = Cursors.WaitCursor;
                CommonClass.ItemSalesAnalysis.MdiParent = this.MdiParent;
                CommonClass.ItemSalesAnalysis.Show();
                CommonClass.ItemSalesAnalysis.Focus();
                if (CommonClass.ItemSalesAnalysis.DialogResult == DialogResult.Cancel)
                {
                    CommonClass.ItemSalesAnalysis.Close();
                }
                this.Cursor = Cursors.Default;
            }
            if (listInventoryReports.SelectedIndex == 19)
            {
                if (CommonClass.CategorySalesAnalysis == null
                           || CommonClass.CategorySalesAnalysis.IsDisposed)
                {
                    CommonClass.CategorySalesAnalysis = new Reports.InventoryReports.RptSalesAnalysisCategory();
                }
                this.Cursor = Cursors.WaitCursor;
                CommonClass.CategorySalesAnalysis.MdiParent = this.MdiParent;
                CommonClass.CategorySalesAnalysis.Show();
                CommonClass.CategorySalesAnalysis.Focus();
                if (CommonClass.CategorySalesAnalysis.DialogResult == DialogResult.Cancel)
                {
                    CommonClass.CategorySalesAnalysis.Close();
                }
                this.Cursor = Cursors.Default;
            }
