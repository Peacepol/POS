...
{
    public partial class RptSalesAnalysisCategory : Form
    {
        ...
        public RptSalesAnalysisCategory()
        {
            InitializeComponent();
        ...
        {
            filterCategories();
            selectSql = @"Select  i.CategoryID,c.CategoryCode, c.Description, SUM(sl.SubTotal) as SubTotal , SUM(sl.TotalCost) as TotalCost, SUM(sl.SubTotal - sl.TotalCost) as Profit,'0.00%' as Margin
                        From Items i 
                        INNER JOIN SalesLines sl ON i.ID = sl.EntityID
                        INNER JOIN Sales s On s.SalesID = sl.SalesID
                        INNER JOIN Category c on i.CategoryID = c.CategoryID
                        WHERE SalesType = 'INVOICE' AND s.TransactionDate BETWEEN @sdate AND @edate";
            if (list1 != "")
            {
            ...
            selectSql += " GROUP BY i.CategoryID,c.CategoryCode, c.Description ";
            try
            {
        ...
        private void CalculateTotal(int pSortIndex = 1, string pSortMode = "asc")
        {
            ...
            for (int i = 0; i < TbRep.Rows.Count; i++)
            {
                ...
                rw[4] = TbRep.Rows[i][4];
                rw[5] = TbRep.Rows[i][5];               
                rw[6] = Math.Round(lMargin *100, 2).ToString() + "%";
                TbGrid.Rows.Add(rw);
            ...
            rw[4] = TotalCost;
            rw[5] = TotalProfit;
            rw[6] = Math.Round(TotalProfit/ TotalAmount * 100,2).ToString() + "%";
            TbGrid.Rows.Add(rw);
        }

        private void FormatGrid()
        {
            this.dgReport.Columns[0].HeaderText = "Category ID";
            this.dgReport.Columns[1].HeaderText = "Category Code";
            this.dgReport.Columns[2].HeaderText = "Description";
            this.dgReport.Columns[3].HeaderText = "Total Amount(Ex)";
            this.dgReport.Columns[4].HeaderText = "Total Cost";          
            this.dgReport.Columns[5].HeaderText = "Profit";
            this.dgReport.Columns[6].HeaderText = "Margin";
        ...
        private void btnPrintGrid_Click(object sender, EventArgs e)
        {
            ...
            dgPrinter.SubTitle = "Category Sales Analysis Report";
            ...
            dgPrinter.ColumnWidths.Add("Category ID", 90);
            dgPrinter.ColumnWidths.Add("Category Code", 90);
            dgPrinter.ColumnWidths.Add("Description", 90);
            dgPrinter.ColumnWidths.Add("TotalAmount", 90);
            dgPrinter.ColumnWidths.Add("Total Cost", 70);
        ...
        private void btnExportExcell_Click(object sender, EventArgs e)
        {
            using (SaveFileDialog sdf = new SaveFileDialog() { Filter = "Excel Workbook|*.xls", ValidateNames = true })
            {
                if (sdf.ShowDialog() == DialogResult.OK)
                {
                    ...
                    ws.Cells[4, 2] = "Category Coder";
                    ws.Cells[4, 3] = "Description";
                    ws.Cells[4, 4] = "Total Amount(Ex)";
                    ws.Cells[4, 5] = "Total Cost";
                    ws.Cells[4, 6] = "Profit";
                    ws.Cells[4, 7] = "Margin";
                    ...
                    foreach (DataGridViewRow item in dgReport.Rows)
                    {
                        ...
                        {
                            double TotalAmount = double.Parse(item.Cells[3].Value.ToString());
                            ws.Cells[i, 4] = TotalAmount.ToString("C");
                        }
                        if (item.Cells[4].Value != null && item.Cells[4].Value.ToString() != "")
                        {
                            double TotalCost = double.Parse(item.Cells[4].Value.ToString());
                            ws.Cells[i, 5] = TotalCost.ToString("C");
                        }
                        if (item.Cells[5].Value != null && item.Cells[5].Value.ToString() != "")
                        {
                            double TotalProfit = double.Parse(item.Cells[5].Value.ToString());
                            ws.Cells[i, 6] = TotalProfit.ToString("C");
                        }
                    ...
                    ws.Cells[1, 1] = "Category Sales Analysis";
                    ...
                    app.Quit();
                    MessageBox.Show("Category Sales Analysis Report has been successfully exported", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    this.Close();
                }
            }
        }
