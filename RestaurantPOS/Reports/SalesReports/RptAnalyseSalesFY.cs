...
namespace AbleRetailPOS.Reports.SalesReports
{
    public partial class RptAnalyseSalesFY : Form
    {
        ...
        private void RptAnalyseSalesFY_Load(object sender, EventArgs e)
        {
            if (!CanView)
            {
        ...
        private void GetReportData()
        {
            ...
            try
            {
                ...
                string sql = @"SELECT d1.ProfileIDNumber, d1.Name, d1.GrandTotal, ISNULL(d2.GrandTotal,0) as GrandTotal2, (ISNULL(d2.GrandTotal,0) - d1.GrandTotal) as diffamount, CAST(0 as float) as diffpercent  
                    from (SELECT cs.* from (SELECT p.ID, p.ProfileIDNumber, p.Name, sum(GrandTotal) as GrandTotal FROM Sales s
                    INNER JOIN Profile p ON s.CustomerID = p.ID WHERE TransactionDate BETWEEN @sdate AND @edate and SalesType in ('INVOICE','SINVOICE') group by p.ID, p.ProfileIDNumber, p.Name ) cs ) d1 
                    LEFT JOIN 
                    (SELECT cs.* from (SELECT p.ID,  sum(GrandTotal) as GrandTotal FROM Sales s
                    INNER JOIN Profile p ON s.CustomerID = p.ID WHERE TransactionDate BETWEEN @sdate2 AND @edate2 and SalesType in ('INVOICE','SINVOICE') group by p.ID) cs) d2
                    on d1.ID = d2.ID";
        ...
        void LoadReport(int pViewType = 0)
        {
            ...
            if (pViewType == 0)
            {
                ...
                salesparams.ReportName = "AnalyseSalesDRComparison.rpt";
                salesparams.RptTitle = "Analyse Customer Sales Date Range Comparison";
        ...
        private void FormatGrid()
        {
            ...
            this.dgReport.Columns[2].HeaderText = "Total Sales Range 1";
            this.dgReport.Columns[3].HeaderText = "Total Sales Range 2";
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            using (SaveFileDialog sdf = new SaveFileDialog() { Filter = "Excel Workbook|*.xls", ValidateNames = true })
            {
                if (sdf.ShowDialog() == DialogResult.OK)
                {
                    ...
                    ws.Cells[4, 3] = "Total Sales Range 1";
                    ws.Cells[4, 4] = "Total Sales Range 2";
                    ...
                    ws.Cells[1, 1] = "Analyse Sales Comparison Report";
                    ...
                    app.Quit();
                    MessageBox.Show("Analyse Sales Comparison Report has been successfully exported", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    this.Close();
                }
            }
        }
