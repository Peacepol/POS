...
{
    public partial class RptOpenInvoiceOrder : Form
    {
        ...
        void LoadReport()
        {
            selectSql = @"SELECT TransactionDate, 
                            ...
                            WHERE TransactionDate BETWEEN @sdate AND @edate AND InvoiceStatus = 'Open' ";
            ...
            try
            {
                ...
                reportTitle = "Sales Register [Open Invoices]";
        ...
        private void btnExportExcell_Click(object sender, EventArgs e)
        {
            using (SaveFileDialog sdf = new SaveFileDialog() { Filter = "Excel Workbook|*.xls", ValidateNames = true })
            {
                if (sdf.ShowDialog() == DialogResult.OK)
                {
                    ...
                    ws.Cells[1, 1] = "Open Invoice Report";
                    ...
                    app.Quit();
                    MessageBox.Show("Open Invoice Report has been successfully exported", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    this.Close();
                }
            }
        }

        private void RptOpenInvoiceOrder_Load(object sender, EventArgs e)
        {
            Dictionary<string, Boolean> FormRights;
            CommonClass.UserAccess.TryGetValue(this.Text, out FormRights);
            Boolean outx = false;
