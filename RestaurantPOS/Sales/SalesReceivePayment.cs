...
namespace AbleRetailPOS
{
    public partial class SalesReceivePayment : Form
    {
        ...
        private void SalesReceivePayment_Load(object sender, EventArgs e)
        {
            ...
            RecipientAccountID = CommonClass.DRowPref["TradeDebtorGLCode"].ToString();//CommonClass.DRowLA["ReceivablesChequeID"].ToString();
            ...
            try
            {
                ...
                if (dtopensales.Rows.Count > 0)
                {
                    ...
                    if (InvokeSrc == CommonClass.InvocationSource.CUSTOMER)
                    {
                        ...
                        label5.Visible = true;
                        TotalReceivedAmount.Value = Convert.ToDecimal(dtopensales.Rows[0]["TotalAmount"].ToString());
                    }
                    else
                    {
        ...
        private void LoadTransaction(int pCustomerID = 0)
        {
                ...
                string sql = @"SELECT s.*, 
                                  ...
                                  pmtds.PaymentMethod ";
                       if (RecipientAccountID != "")
                           sql += ", " + RecipientAccountID + " AS AccountID ,t.DiscountDays, t.EarlyPaymentDiscountPercent,t.LatePaymentChargePercent,t.BalanceDueDays,p.EarlyPaymentDiscountPercent as DiscountPercent,p.LatePaymentChargePercent as LateCharge ";
                       sql += @"FROM Sales s 
                              INNER JOIN Profile p ON s.CustomerID = p.ID
                              LEFT JOIN PaymentMethods pmtds ON p.MethodOfPaymentID = pmtds.id
                                Left Join Terms as t ON t.TermsID = s.TermsReferenceID 
                ...
                if (dtopensales.Rows.Count > 0)
                {
                    ...
                    TimeSpan diff = nowdate.Subtract(Convert.ToDateTime(dtRow["TransactionDate"]).ToLocalTime());
                    if (diff.Days <= Convert.ToInt32(dtRow["DiscountDays"]))
                    {
                        dgridRecvPayment["Discount", newrowindex].Value = dtRow["EarlyPaymentDiscountPercent"] != null ? dtRow["EarlyPaymentDiscountPercent"].ToString() : dtRow["DiscountPercent"].ToString();
                    }
                    else if (Convert.ToInt32(dtRow["BalanceDueDays"]) != 0 || diff.Days > Convert.ToInt32(dtRow["BalanceDueDays"]))
                    {
                        dgridRecvPayment["Discount", newrowindex].Value = "-" + dtRow["LatePaymentChargePercent"].ToString() != null ? dtRow["LatePaymentChargePercent"].ToString() : dtRow["LateCharge"].ToString();
                    }
                    //the ff. fields are hidden only used to pull back the data when creating a negative invoice for discounts
                    dgridRecvPayment["SalesTermsReferenceID", newrowindex].Value = dtRow["TermsReferenceID"];
                        dgridRecvPayment["SalesShippingContactID", newrowindex].Value = dtRow["ShippingContactID"];
                        ...
                        dgridRecvPayment["TotalDue", newrowindex].Value = totaldue - (totaldue * (discount / 100));
            ...
            string outx = "";             
                ...
                if (paymentid != 0)
                {
                    ...
                    for (int i = 0; i < PaymentInfoTb.Rows.Count; i++)
                    {
                        ...
                        int ptid = CommonClass.runSql(sqlTender, CommonClass.RunSqlInsertMode.SCALAR, paramTender);
                        ...
                        string sqlDetails = @"SET IDENTITY_INSERT PaymentDetails ON;
                                            INSERT INTO PaymentDetails (  
                                                      ...
                                                      VALUES ( 
                                                        ...
                                                        @PaymentGCNo,
                                                        @PaymentGCNotes)";
                    paramDetails.Add("@PaymentDetailsID", ptid);
                    paramDetails.Add("@PaymentID", paymentid);
                        ...
                        int pdid = CommonClass.runSql(sqlDetails, CommonClass.RunSqlInsertMode.SCALAR, paramDetails);
