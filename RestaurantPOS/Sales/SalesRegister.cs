    ...
    public partial class SalesRegister : Form
    {
        ...
        public void Populatedg(ref DataGridView dgpopulate, string type)
        {
            ...
            Dictionary<string, object> param = new Dictionary<string, object>();
                ...
                displaydata(ref dgpopulate);
            if (dt.Rows.Count > 100)
            {
                btnNextP.Enabled = true;
                int pagecount = dt.Rows.Count /100;
                int reminder = dt.Rows.Count % 100;
                if(reminder!=0){
                    pagecount = pagecount + 1;
                }
                page_label.Text = pageno + " of " + pagecount;
            }
            
            //   
        }
        public void displaydata(ref DataGridView dgpopulate)
        {
            int count = 0;
            for (int x = startindex; x < dt.Rows.Count; x++)
            {
                if(x < endindex)
                {
                    DataRow dr = dt.Rows[x];
                    dgpopulate.Rows.Add();
                    dgpopulate.Rows[count].Cells[0].Value = dr["SalesID"].ToString();
                    dgpopulate.Rows[count].Cells[1].Value = Convert.ToDateTime(dr["TransactionDate"].ToString()).ToLocalTime().ToShortDateString();
                    dgpopulate.Rows[count].Cells[2].Value = dr["SalesNumber"].ToString();
                    dgpopulate.Rows[count].Cells[3].Value = dr["CustomerPONumber"].ToString();
                    dgpopulate.Rows[count].Cells[4].Value = dr["Name"].ToString();
                    dgpopulate.Rows[count].Cells[5].Value = dr["GrandTotal"].ToString();
                    dgpopulate.Rows[count].Cells[6].Value = dr["TotalDue"].ToString();
                    dgpopulate.Rows[count].Cells[7].Value = (dr["PromiseDate"] == DBNull.Value ? null : Convert.ToDateTime(dr["PromiseDate"].ToString()).ToLocalTime().ToShortDateString());
                    dgpopulate.Rows[count].Cells[8].Value = dr["SalesType"].ToString();
                    dgpopulate.Rows[count].Cells[10].Value = dr["ClosedDate"].ToString() != "" ? Convert.ToDateTime(dr["ClosedDate"].ToString()).ToLocalTime().ToShortDateString() : "";
                    count++;
                }
                else
                {
        ...
        private void dgOrder_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (CommonClass.SessionID == 0)
            {
                MessageBox.Show("Session is needed to open", "Information");
                return;
            }
        ...
        private void toInvoice_btn_Click(object sender, EventArgs e)
        {
            ...
            if(index >= 0)
            {
                ...
                {
                    Sid = dgLayby.Rows[index].Cells[0].Value.ToString();
                }
        ...
        private void dgSalesRegAll_Scroll(object sender, ScrollEventArgs e)
        {
        }
        ...
        private void btnNextP_Click(object sender, EventArgs e)
        {
            ...
            switch (TCSaleReg.SelectedIndex)
            {
                case 0:
                    dgSalesRegAll.Rows.Clear();
                    displaydata(ref dgSalesRegAll);
                    break;
                case 1:
                    dgQuote.Rows.Clear();
                    displaydata(ref dgQuote);

                    break;
                case 2:
                    dgOrder.Rows.Clear();
                    displaydata(ref dgOrder);
                    break;
                case 3:
                    dgLayby.Rows.Clear();
                    displaydata(ref dgLayby);
                    break;
                case 4:
                    dgOInvoice.Rows.Clear();
                    displaydata(ref dgOInvoice);

                    break;
                case 5:
                    dgRetCredit.Rows.Clear();
                    displaydata(ref dgRetCredit);
                    break;

                case 6:
                    dgCInvoice.Rows.Clear();
                    displaydata(ref dgCInvoice);
                    break;
            }
