    ...
    public partial class EnterSales : Form
    {
        ...
        private void EnterSales_Load(object sender, EventArgs e)
        {
            ...
            if (XsalesID != "")
            {
                ...
                if (!CommonClass.IsEditOK)
                {
                        ...
                        && SrcOfInvoke != CommonClass.InvocationSource.USERECURRING && SrcOfInvoke != CommonClass.InvocationSource.REMINDER)
                    {
                        ...
                        record_btn.Enabled = false; //disable Save Record as Transaction Editable is false;
                        if (salestype_cb.Text == "QUOTE")
                        {
                            record_btn.Enabled = CanEdit; //ENABLED IF QUOTE
                        }
                    }
            ...
            else
            {
                ...
                LoadDefaultCustomer();
                btnVoid.Visible = false;//NEW SALE TRANS do not show VOID button
            ...
            if (dgEnterSales.Rows.Count > 0)
            {
                foreach (DataGridViewRow dgvr in dgEnterSales.Rows)
                {
                    if (dgvr.Cells["PartNumber"].Value != null)
                    {
                        ...
                        {
                            if (dgvr.Cells["Price"].Value != null && dgvr.Cells["Price"].Value.ToString() != "0")
                            {
                                shipCount += 1;
        ...
        public bool ShowItemLookup(string itemNum, string whereCon,float qty = 0,float pricex= 0, int lastindex = 0)
        {
            ...
            if (lastindex != 0)
            {
               dgEnterSales.Rows[lastindex].Selected = true;
               dgvRows = dgEnterSales.Rows[lastindex];
            }
            //dgEnterSales.Rows[lLastIndex].Selected = true;
            //dgvRows = dgEnterSales.Rows[lLastIndex];
            ...
            if (Items.ShowDialog() == DialogResult.OK)
            {
                ...
                if (!CheckExistItemInGrid(ItemRows.Cells[1].Value.ToString()))
                {
                    ...
                    if(pricex > 0) // this is if price was from the import file
                    {
                        dgvRows.Cells["Price"].Value = pricex;
                        //dgvRows.Cells["ActualUnitPrice"].Value = pricex;
                    }
                ...
                foreach (DataRow x in dtPromo.Rows)
                {
                    ...
                    if (includepromo)
                    {
                        foreach (RuleCriteriaPoints promo in itemPromos)
                        {
                            ...
                            {
                                if (dgEnterSales.CurrentRow.Cells["Supplier"].Value != null){
                            ...
                            {
                                if (dgEnterSales.CurrentRow.Cells["CategoryID"].Value != null)
                                {
                            ...
                            {
                                if (dgEnterSales.CurrentRow.Cells["Brand"].Value != null)
                                {
        ...
        private void dgEnterSales_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //CHECK IF NEEDED TO ADD A NEW BLANK ROW
            int lLastRow = dgEnterSales.Rows.Count - 1;
            if(dgEnterSales.Rows[lLastRow].Cells[1].Value != null)
            {
                dgEnterSales.Rows.Add();
            }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (OverrideCreditLimit == DialogResult.Yes)
                            {
                                ...
                                if(!CommonClass.isSupervisor || !CommonClass.isAdministrator)
                                {
                                    VoidValidation DlgVoid = new VoidValidation();
                                    if (DlgVoid.ShowDialog() == DialogResult.OK)
                                    {
        ...
        public int SaveSale(bool pIsRecurring = false)
        {
            if (customerText.Text != "" && checkSale())
            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < this.dgEnterSales.Rows.Count; i++)
                    {
                        if (dgEnterSales.Rows[i].Cells["ItemID"].Value != null)
                        {
                            if (dgEnterSales.Rows[i].Cells["ItemID"].Value.ToString() != "")
                            {
                                ...
                                paramSalesLine.Add("@TransactionDate", salesDate.Value.ToUniversalTime());
                    ...
                    //UpdateItemQty();
                    SaveIngredientData(NewSalesID.ToString());
        ...
        private void SaveIngredientData(string pSalesID)
        {
            string lTranDate = salesDate.Value.ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ss");
            if (IngredientTb.Rows.Count > 0)
            {
                foreach (DataRow dr in IngredientTb.Rows)
                {
                    if (dr["ItemID"] != null)
                    {
                        if (dr["ItemID"].ToString() != "")
                        {
                            ...
                            paramIngredient.Add("@SalesID", pSalesID);
                            ...
                            if (salestype_cb.Text == "INVOICE")
                            {
                                ...
                                if (lItemTb.Rows.Count > 0)
                                {
                                    ...
                                    if (lIsCounted)
                                    {
                                        ...
                                        // INSERT SOLD ITEMS IN ITEM TRANSACTION
                                        salesLinesql = @"INSERT INTO ItemTransaction(TransactionDate,ItemId,TransactionQty,QtyAdjustment,CostEx,TotalCostEx,TranType,SourceTranID,UserID) 
                                                    VALUES('" + lTranDate + "'," + dr["ItemID"].ToString() + "," + lTranQty + "," + lTranQty + " * (-1) ," + CostPrice + "," + TotalCost + ",'SI'," + pSalesID + "," + CommonClass.UserID + ")";
        ...
        void UpdateSale(string pOldSalesNo)
        {
            ...
            if (salestype_cb.Text == "INVOICE")
            {
                ...
                ReverseIngredientsItemQty(XsalesID);
            }
                ...
                if (EditSalesRecord(XsalesID, invoicenum))
                {
                    if (salestype_cb.Text == "INVOICE")
                    {
                        ...
                        if (PaidToday_txt.Value == PrevPaid && PrevPaid != 0)
                        {
                            ...
                            PaymentCommon.UpdateSalesRecord(DateTime.Now.ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ss"), invoicenum, PaidToday_txt.Value);
                        }
                    }
                    if (salestype_cb.Text == "ORDER")
                    {
            ...
            try
            {
                ...
                for (int i = 0; i < lItemTb.Rows.Count; i++)
                {
                    ...
                    if (lIsCounted)
                    {
                        ...
                        sql = "UPDATE ItemsQty set OnHandQty = OnHandQty - " + lTranQty.ToString() + " where ItemID = " + entity;
                        cmd.CommandText = sql;
                        count = cmd.ExecuteNonQuery();
        ...
        public void ReverseIngredientsItemQty(string pSalesID)
        {
            SqlConnection con = null;
            try
            {
                string sql = @"Select ing.*,  i.IsCounted, i.IncomeAccountID, i.IsSold, i.COSAccountID, i.IsBought   from Ingredient ing inner join Items i on ing.ItemID = i.ID  where ing.SalesID = " + pSalesID;
                con = new SqlConnection(CommonClass.ConStr);
                SqlCommand cmd = new SqlCommand(sql, con);
                con.Open();
                ...
                {
                    bool lIsCounted = (bool)lItemTb.Rows[i]["IsCounted"];
                    float lQty = float.Parse(lItemTb.Rows[i]["Qty"].ToString()) * -1;
                    float lQtySUOM = float.Parse(lItemTb.Rows[0]["QtyPerSellingUnit"].ToString());
                    float lTranQty = lQty * lQtySUOM;
                    if (lIsCounted)
                    {
                        float CostPrice = (lItemTb.Rows[i]["Cost"] == null || lItemTb.Rows[i]["Cost"].ToString() == "" ? 0 : float.Parse(lItemTb.Rows[i]["Cost"].ToString()));
                        float TotalCost = float.Parse(lItemTb.Rows[i]["TotalCost"].ToString());
                        string itemID = lItemTb.Rows[i]["ItemID"].ToString();
                        string lTranDate = salesDate.Value.ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ss");

                        sql = "UPDATE ItemsQty set OnHandQty = OnHandQty - " + lTranQty.ToString() + " where ItemID = " + itemID;
                        cmd.CommandText = sql;
                        count = cmd.ExecuteNonQuery();
                        //DELETE ENTRY IN ITEM TRANSACTION
                        sql = "DELETE FROM ItemTransaction where TranType = 'SII' and SourceTranID = " + pSalesID;
                        cmd.CommandText = sql;
                        count = cmd.ExecuteNonQuery();
                    }
                }
        ...
        private bool EditSalesRecord(string pSalesID, string pSalesNo, decimal pPaidToday = 0)
        {
            SqlConnection con_ = null;
            try
            {
                ...
                cmd.Parameters.AddWithValue("@TotalPaid", pPaidToday);
                cmd.Parameters.AddWithValue("@TotalDue", BalanceDue_txt.Value);
                cmd.Parameters.AddWithValue("@FreightTaxCode", txtFTaxCode.Text);
                cmd.Parameters.AddWithValue("@FreightTaxRate", FreightTaxRate);
                cmd.Parameters.AddWithValue("@SalesPersonID", SalesPersonID);
                ...
                for (int i = 0; i < this.dgEnterSales.Rows.Count; i++)
                {
                    if (this.dgEnterSales.Rows[i].Cells["ItemID"].Value != null)
                    {
                        if (this.dgEnterSales.Rows[i].Cells["ItemID"].Value.ToString() != "")
                        {
                            ...
                            TaxRate = dgEnterSales.Rows[i].Cells["TaxRate"].Value == null ? "0" : dgEnterSales.Rows[i].Cells["TaxRate"].Value.ToString();
                            double taxEx = Convert.ToDouble(dgEnterSales.Rows[i].Cells["TaxExclusiveAmount"].Value.ToString());
                            double taxInc = Convert.ToDouble(dgEnterSales.Rows[i].Cells["TaxInclusiveAmount"].Value.ToString());
                            double TaxAm = taxInc - taxEx;
                            ...
                            if (salestype_cb.Text == "INVOICE")
                            {
                                ...
                                if (lItemTb.Rows.Count > 0)
                                {
                                    ...
                                    if (lIsCounted)
                                    {
                                        ...
                                        // INSERT SOLD ITEMS IN ITEM TRANSACTION
                                        salesLinesql = @"INSERT INTO ItemTransaction(TransactionDate,ItemId,TransactionQty,QtyAdjustment,CostEx,TotalCostEx,TranType,SourceTranID,UserID) 
                                            VALUES(@TransDate," + entity + "," + lTranQty + "," + lTranQty + " *(-1)," + CostPrice + "," + TotalCost + ",'SI'," + pSalesID + "," + CommonClass.UserID + ")";
                                        cmd.CommandText = salesLinesql;
                                        count = cmd.ExecuteNonQuery();
                                    }
                                }
                ...
                SaveIngredientData(XsalesID.ToString());
  ...
  FROM Ingredient ing inner join Items i on ing.ItemID = i.ID where ing.SalesID = " + pSalesID;
                ...
                if (ltb.Rows.Count > 0)
                {
                    ...
                    if (lGrandTotal < 0)
                    {
                        ...
                        sql = "INSERT INTO Journal(TransactionDate, Memo, AllocationMemo,AccountID, CreditAmount, TransactionNumber, Type, EntityID)  " +
                             ...
                             (lGrandTotal * -1).ToString() + ",'" + lSalesNum + "', 'SI', " + lSalesID + ")";
                    }
                    else
                    {
                        //DEBIT AMOUNT
                        sql = "INSERT INTO Journal(TransactionDate, Memo, AllocationMemo, AccountID, DebitAmount, TransactionNumber, Type, EntityID) " +
                               " VALUES('" + lTranDate + "',@lMemo,@lMemo,'" + lRecipientID + "', " +
                              lGrandTotal + ",'" + lSalesNum + "','SI', " + lSalesID + ")";
                    }
                    cmd.Parameters.Add("@lMemo", lMemo);
                    cmd.CommandText = sql;
                    cmd.ExecuteNonQuery();
        ...
        VALUES('" + lTranDate + "', @lMemo, @lMemo ,'" + lAccountID + "'," + (lTaxEx * -1) + ",'" + lSalesNum + "', 'SI','" + lJobID + "','" + lEntity + "')";
                        ...
                        else //POSITIVE SO CREDIT AMOUNT 
                        {
                            ...
                            {
                                sql = @"INSERT INTO Journal(TransactionDate, Memo, AllocationMemo,AccountID,
                                                    CreditAmount, TransactionNumber, Type, JobID, EntityID) VALUES
            ('" + lTranDate + "', @lMemo, @lMemo ,'" + lTaxCollectedAccountID + "'," + lTaxAmt.ToString() + ",'" + lSalesNum + "', 'SI'," + lJobID + ",'" + lSalesID + "')";
                                cmd.CommandText = sql;
                                cmd.ExecuteNonQuery();
                            }
                            if (lIsCounted)
        ...
        private void LoadTransaction()
        {
            ...
            try
            {
                ...
                if (TbRepSales.Rows.Count > 0)
                {
                    ...
                    sql = @"SELECT ap.RedeemID,ap.RedemptionType,l.*, j.*, i.PartNumber, i.ItemNumber, ISNULL(ap.PointsAccumulated,0) as PointsAccumulated, ISNULL(ap.CustomerID,0) as lMemberID, ISNULL(lm.Number,'') as lMemberNo
                            FROM Sales s 
                    ...
                    for (int i = 0; i < TbRepSalesLines.Rows.Count; i++)
                    {
                        ...
                        if (TbRepSalesLines.Rows[i]["RedeemID"].ToString() != "")
                        {
                            DRow.Cells["RedeemID"].Value = TbRepSalesLines.Rows[i]["RedeemID"].ToString();
                            DRow.Cells["RedemptionType"].Value = TbRepSalesLines.Rows[i]["RedemptionType"].ToString();
                        }
        ...
        private void LoadItemLayoutReport(string pSalesID)
        {
            if (cmbPrintFormat.SelectedIndex == 0)
            {
                ...
                SqlConnection con = new SqlConnection(CommonClass.ConStr);
                string printSale = @"SELECT SalesType, SalesNumber , Sales.TransactionDate  , Sales.Comments, Sales.SubTotal , Sales.CustomerPONumber, p.Name as ProfileName,
                   ...
                   c2.Street as BillStreet, c2.City as BillCity, c2.State as BillState, 
                    ...
                    ShippingMethod, i.ItemNumber, sl.ShipQty, sl.OrderQty, sl.CostPrice, sl.UnitPrice, sl.DiscountPercent, u.user_fullname,i.ItemDescription, i.SellingUOM
                    FROM Sales
                    INNER JOIN Profile p on Sales.CustomerID = p.ID
                    Left JOIN ShippingMethods  sm On sm.ShippingID = Sales.ShippingMethodID
                    LEFT JOIN Contacts c ON c.ContactID = Sales.ShippingContactID and c.ProfileID = Sales.CustomerID  
                    LEFT JOIN Contacts c2 ON c2.ContactID = Sales.BillingContactID and c2.ProfileID = Sales.CustomerID 
                    INNER JOIN SalesLines sl ON sl.SalesID = Sales.SalesID
                    INNER JOIN Items i ON sl.EntityID = i.ID
            ...
            else 
            {
                ...
                SqlConnection con = new SqlConnection(CommonClass.ConStr);
                string printSale = "";
                if (cmbPrintFormat.SelectedIndex == 1)
                {
                    printSale = @"SELECT SalesType, SalesNumber , Sales.TransactionDate  , Sales.Comments, Sales.SubTotal , p.Name as ProfileName,
                    c.Street as ShipStreet, c.City as ShipCity, c.State as ShipState,
                   c2.Street as BillStreet, c2.City as BillCity, c2.State as BillState, 
                    sl.Description ,sl.TotalAmount, sl.TaxCode,Sales.TaxTotal,FreightTax,TotalPaid, GrandTotal,TotalDue,
                    ShippingMethod, i.ItemNumber, sl.ShipQty, sl.OrderQty, sl.CostPrice, sl.UnitPrice, sl.DiscountPercent, u.user_fullname, i.SellingUOM
                    FROM Sales
                    INNER JOIN Profile p on Sales.CustomerID = p.ID
                    Left JOIN ShippingMethods  sm On sm.ShippingID = Sales.ShippingMethodID
                    LEFT JOIN Contacts c ON c.ContactID = Sales.ShippingContactID and c.ProfileID = Sales.CustomerID  
                ...
                else
                {
                   ...
                   c2.Street as BillStreet, c2.City as BillCity, c2.State as BillState, 
                    sl.Description ,sl.TotalAmount, sl.TaxCode,Sales.TaxTotal,FreightTax,TotalPaid, GrandTotal,TotalDue,
                    ShippingMethod, i.ItemNumber, sl.ShipQty, sl.OrderQty, sl.CostPrice, sl.UnitPrice, sl.DiscountPercent, u.user_fullname, i.SellingUOM,  Sales.SalesID, ISNULL(Sales.TableNumber,'') as TableNumber
                    FROM Sales
                    INNER JOIN Profile p on Sales.CustomerID = p.ID
                    Left JOIN ShippingMethods  sm On sm.ShippingID = Sales.ShippingMethodID
                    LEFT JOIN Contacts c ON c.ContactID = Sales.ShippingContactID and c.ProfileID = Sales.CustomerID  
        ...
        private void btnRedeem_Click(object sender, EventArgs e)
        {
            ...
            {
                CommonClass.PointRedemption = new PointsRedemption(CommonClass.RedemptionType.GIFTCERTIFICATE, null, this, null, CustomerID, customerText.Text);
            }
        ...
        void LoadDefaultCustomer()
        {
            ...
            {
                DataTable dt = new DataTable();
                string sqldefcust = "Select p.*, isnull(s.ShippingID,0) as ShippingID FROM Profile p INNER JOIN Contacts c ON p.ID = c.ProfileID  LEFT JOIN ShippingMethods s on p.ShippingMethodID = s.ShippingMethod WHERE ID = " + CommonClass.DefaultCustomerID;
        ...
        private void txtBarcode_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                for (int x = 0; x < dt.Rows.Count; x++)
                {
                    ...
                    if (txtBarcode.Text ==dr["BarcodeData"].ToString())
                    {
        ...
        private void btnLoyaltyMem_Click(object sender, EventArgs e)
        {
            ...
            {
                ismember = true;
                memberID = loyaldt.Rows[0]["ProfileID"].ToString();
        ...
        private void VoidSale(string pSalesID, string pSalesNo)
        {
            ...
            //REVERSE QTY
            ReverseItemQty(pSalesID);
            ReverseIngredientsItemQty(pSalesID);
            ...
            sql = "DELETE from ItemTransaction where TranType  in ('SI','SII') and SourceTranID = " + pSalesID;
                    ...
                    if (dlg.FileName.EndsWith(".csv") || dlg.FileName.EndsWith(".txt"))
                    {
                        ...
                        if (dt.Rows != null && dt.Rows.ToString() != String.Empty)
                        {
                            ...
                            if (columns.Contains("PartNumber") && columns.Contains("Quantity"))
                            {
                                foreach (DataRow dr in dt.Rows)
                                {
                                    ...
                                    if (index < 0)
                                    {
                                        ...
                                        if (checkitemexist(dr["PartNumber"].ToString()))
                                        {
                                            ...
                                            {                                              
                                                ...
                                                {
                                                    float number = float.Parse(dr["Price"].ToString());
                                                    ShowItemLookup(dr["PartNumber"].ToString(), "PartNumber", float.Parse(dr["Quantity"].ToString()), number, lastindex);
                                                }
                                                else
                                                {
                                                    ShowItemLookup(dr["PartNumber"].ToString(), "PartNumber", float.Parse(dr["Quantity"].ToString()),0, lastindex);
                                                }
                                            }
                                            else
                                            {
                                    ...
                                    else
                                    {
                                        if (columns.Contains("Price"))
                                        {
                                            ...
                                            {
                                                float number = float.Parse(dr["Price"].ToString());
                                                dgEnterSales.Rows[index].Cells["Price"].Value = number;
                                            }
                                        }
                                        dgEnterSales.Rows[index].Cells["Ship"].Value = float.Parse(dr["Quantity"].ToString());
                                        itemcalc(index);
                                        Recalcline(8, index);
                                        CalcOutOfBalance();
                                    }
                                ...
                                if (lLastIndex < dgEnterSales.Rows.Count)
                                {
                                    dgEnterSales.FirstDisplayedScrollingRowIndex = lLastIndex;
                                }
                                dgEnterSales.ClearSelection();
                                dgEnterSales.Rows.Add();
        ...
        private int GetLastIndex()
        {
            ...
            for (int i = lLastIndex; i > -1; i--)
            {
                if (dgEnterSales.Rows[i].Cells["PartNumber"].Value == null)
                {
                    if (i != 0)
                    {
                        if (dgEnterSales.Rows[i - 1].Cells["PartNumber"].Value != null)
                        {
                            lNextIndex = i;
                            break;
                        }
