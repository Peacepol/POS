...
namespace AbleRetailPOS
{
    public partial class PointsRedemption : Form
    {
        ...
        private Sales.RestoPOS mRSaleRef = null;
        private float pointexchangeRate;
        private float amountValue;

        public PointsRedemption(CommonClass.RedemptionType pRtype, Sales.RestoPOS restoref, Sales.EnterSales saleref, Sales.QuickSales qsaleref, string pCustID = "", string pCustName = "")
        {
            mSaleRef = saleref;
            mQSaleRef = qsaleref;
            mRSaleRef = restoref;
        ...
        private void ShowRedemptionItems()
        {
            ...
            switch (mRedeemType)
            {
                case CommonClass.RedemptionType.GIFTCERTIFICATE:
                    sql = @"SELECT i.ID,
                            ...
                            AND pi.requiredpoints <= @TotalPoints AND gc.issuedSalesID is null";
                    break;
            ...
            if (mRedeemType != CommonClass.RedemptionType.PRICEDISCOUNT)
            {
                ...
                {
                    //This is causing an error just hide it.
        ...
        private void btnRedeem_Click(object sender, EventArgs e)
        {
            if (cmbRedeemType.Text != "Price Discount")
            {
                ...
                if (mDgvRow != null)
                {
                    if (mSaleRef != null || mQSaleRef != null || mRSaleRef != null)
                    {
                        ...
                        if (mRSaleRef != null)
                        {
                            lSalesLineGrid = mRSaleRef.GetSalesLinesGridView;
                        }
                        lSalesLineGrid.Rows.Add();
                        foreach (DataGridViewRow ldgvRow in lSalesLineGrid.Rows)
                        {
                            if (ldgvRow.Cells["ItemID"].Value == null)
                            {
                                ...
                                ldgvRow.Cells["Description"].Value = "Redeemed Item - " + mDgvRow.Cells["ItemName"].Value.ToString();
                                ...
                                ldgvRow.Cells["Price"].Value = 0;
                                ldgvRow.Cells["TaxExclusiveAmount"].Value = 0;
                                ldgvRow.Cells["TaxInclusiveAmount"].Value = 0;
                                ...
                                {
                                    mRSaleRef.Recalcline(8, ldgvRow.Index);
                                    mRSaleRef.CalcOutOfBalance();
                                }
            ...
            else
            {
                if (mSaleRef != null || mQSaleRef != null || mRSaleRef != null)
                {
                    string pdstring= "SELECT * FROM Items WHERE ID = '1'";
                    DataTable dtpd = new DataTable();
                    CommonClass.runSql(ref dtpd, pdstring);
                    ...
                    foreach (DataGridViewRow ldgvRow in lSalesLineGrid.Rows)
                    {
                        if (ldgvRow.Cells["ItemID"].Value == null)
                        {
                            ldgvRow.Cells["ItemID"].Value = dtpd.Rows[0]["ID"].ToString(); ;
                            ldgvRow.Cells["PartNumber"].Value = dtpd.Rows[0]["PartNumber"].ToString();// "";
                            ldgvRow.Cells["Description"].Value = "Redeemed Item - Price discount for points";
                            ldgvRow.Cells["JobID"].Value = 0;
                            ldgvRow.Cells["BackOrder"].Value = 0;
                            ldgvRow.Cells["Ship"].Value = -1;
                            ldgvRow.Cells["TaxRate"].Value = 0;
                            DataRow rTx = CommonClass.getTaxDetails(dtpd.Rows[0]["SalesTaxCode"].ToString());
                            string taxcode = dtpd.Rows[0]["SalesTaxCode"].ToString();
                            if (taxcode != "")
                            {
                                ldgvRow.Cells["TaxRate"].Value = float.Parse(rTx["TaxPercentageRate"].ToString() == "" ? "0" : rTx["TaxPercentageRate"].ToString());
                                ldgvRow.Cells["TaxCode"].Value = taxcode;
                                ldgvRow.Cells["TaxCollectedAccountID"].Value = ((rTx["TaxCollectedAccountID"] == null || rTx["TaxCollectedAccountID"].ToString() == "") ? "0" : rTx["TaxCollectedAccountID"].ToString());
                            }
                            else
                            {
                                ldgvRow.Cells["TaxRate"].Value = 0;
                            ...
                            ldgvRow.Cells["RedemptionType"].Value = cmbRedeemType.Text;
                            ldgvRow.Cells["RedeemID"].Value = -1;
                            if (mSaleRef != null)
                            {
                                mSaleRef.Recalcline(8, ldgvRow.Index);
                                mSaleRef.CalcOutOfBalance();
