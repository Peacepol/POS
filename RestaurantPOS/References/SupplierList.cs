        ...
        private void LoadSuppliers(string pSearch = "")
        {
            ...
            try
            {
                ...
                if (pSearch != "")
                {
                    ...
                    {
                        sqlcon = " AND Name LIKE @txtsearch";
                    }
                    if (this.rdoID.Checked)
                    {
                        sqlcon = " AND ProfileIDNumber LIKE @txtsearch";
                    }
                    if (this.rdoPhone.Checked)
                    {
                        sqlcon = " AND Phone LIKE @txtsearch";
                    }
                    if (this.rdoEmail.Checked)
                    {
                        sqlcon = " AND Email LIKE @txtsearch";
                    }
                    if (this.rdoCity.Checked)
                    {
                        sqlcon = " AND City LIKE @txtsearch";
                    }
                    if (this.rdoState.Checked)
                    {
                        sqlcon = " AND State LIKE @txtsearch";
                    }
                    if (this.rdoPostCode.Checked)
                    {
                        sqlcon = " AND Postcode LIKE @txtsearch";
                    }
                    if (this.rdoCountry.Checked)
                    {
                        sqlcon = " AND Country LIKE @txtsearch";
                    }
                }
                selectSql += sqlcon;
                SqlCommand cmd_ = new SqlCommand(selectSql, con_);
                con_.Open();
                cmd_.Parameters.AddWithValue("@txtsearch", "%" + this.txtsearch.Text + "%");
